...
| **総 Type（型定義）数** | **780** 型 | 金融商品、法的契約、ライフサイクル、市場データ等のデータ構造 |
| **総 Function（関数定義）数** | **1,323** 関数 | FpML マッピング、商品・イベント自動判定、ライフサイクル処理、計算等 |
| **総 Enum（列挙型）数** | **280** 列挙型 | コードリスト、ビジネスセンター、金融用語定数等 |
...
|---|---|---|---|---|---|
| **`ingest-fpml-`** | 44 | **0** | **819** | 0 | FpML XML メッセージから CDM へのデータ抽出・変換・マッピング関数群 |
| **`product-`** | 24 | **218** | **188** | 56 | 金融商品（IRS, CDS, オプション, コモディティ等）、ペイアウト、決済、適格性判定 |
| **`legaldocumentation-`** | 17 | **248** | **5** | 74 | ISDA/ICMA/ISLA マスター契約、CSA 担保契約、取引追記条項 |
| **`base-`** | 26 | **118** | **100** | 75 | 日時、営業日、日数計算、数学関数、当事者(Party)、識別子、共通資産データ |
| **`event-`** | 16 | **112** | **158** | 35 | 取引イベント（約定、更改、清算等）、状態遷移(TradeState)、ワークフロー、ポジション |
| **`observable-`** | 15 | **81** | **30** | 38 | 参照金利(FRO: SOFR/TONA等)、複利計算、市場観測イベント、価格・数量(PriceQuantity) |
| **`margin-schedule-`** | 3 | **3** | **23** | 2 | 規制証拠金（Initial Margin / Variation Margin）計算スケジュール |
| **合計** | **145** | **780** | **1,323** | **280** | |

---

## 3. Type（型定義：780件）のビジネス機能別分類

```mermaid
pie title Rosetta DSL Type (780件) の機能別構成比
    "法的文書・契約・担保 (Legal & CSA)" : 248
    "金融商品・ペイアウト (Product Modeling)" : 218
    "基礎基盤・共通データ (Base & Static Data)" : 118
    "取引イベント・ワークフロー (Event & Workflow)" : 112
    "市場データ・観測 (Observable & Market Data)" : 81
    "マージンスケジュール (Margin Schedule)" : 3
```

### 3.1 法的文書・契約・担保 (Legal Documentation & CSA): 248 型 (31.8%)
- **CSA 担保契約** (`cdm.legaldocumentation.csa`): **165 型**（適格担保資産、ヘアカット、担保評価条件等）
...
### 3.2 金融商品・ペイアウト (Product Modeling & Payout): 218 型 (27.9%)
- **商品テンプレート・構造** (`cdm.product.template`): **61 型**（`TradableProduct`, `EconomicTerms`, `Payout` 等）
- **資産・ペイアウト構造** (`cdm.product.asset`): **54 型**（`InterestRatePayout`, `EquityPayout`, `CommodityPayout` 等）
- **担保仕様** (`cdm.product.collateral`): **40 型**（`Collateral`, `CollateralPortfolio` 等）
...
### 3.3 基礎基盤・共通データ (Base & Static Data): 118 型 (15.1%)
- **共通資産参照** (`cdm.base.staticdata.asset.common`): **33 型**（`Asset`, `Security`, `Commodity` 等）
...
### 3.4 取引イベント・ライフサイクル (Lifecycle & Workflow): 112 型 (14.4%)
- **取引イベント共通** (`cdm.event.common`): **70 型**（`TradeState`, `BusinessEvent`, `Trade`, `Instruction` 等）
- **命令合成・リセット処理** (`cdm.event.instructioncomposition*`): **15 型**（`cdm.event.instructioncomposition` 7型、`cdm.event.instructioncomposition.reset` 8型: `AdjustPeriodInstruction`, `AdjustDateInstruction` 等）
...
### 3.5 市場データ・観測・価格 (Observable & Market Data): 81 型 (10.4%)
- **価格・数量・市場観測値** (`cdm.observable.asset`): **47 型**（`Price`, `PriceQuantity`, `Observable` 等）
...
## 4. Function（関数定義：1,323件）の目的・機能別分類

```mermaid
pie title Rosetta DSL Function (1,323件) の機能別構成比
    "FpML 取込マッピング (Ingestion)" : 819
    "自動適格性判定 (Qualification)" : 147
    "取引イベント生成・処理 (Lifecycle)" : 113
    "商品・共通ユーティリティ (Utility)" : 111
    "金利・日数・数値計算 (Calculation)" : 80
    "担保・マージン評価 (Collateral & Margin)" : 43
    "ワークフロー・ポジション (Workflow & Position)" : 10
```

### 4.1 外部データ取込・FpML マッピング (Ingestion / Mapping): 819 関数 (61.9%)
FpML XML 構造から CDM オブジェクトへのデータ抽出および型安全な変換を担う関数群。
- **価格・数量・単価マッピング** (`cdm.ingest.fpml.confirmation.pricequantity`): **124 関数**
- **その他データマッピング** (`cdm.ingest.fpml.confirmation.other`): **100 関数**（当事者識別子 `MapPersonIdentifierTypeEnum` 等）
...
- **法務・ワークフロー・メッセージ・状態取込**: **51 関数**（`MapTradeToExecutionInstruction` 等）

### 4.2 自動適格性判定 (Qualification Functions): 147 関数 (11.1%)
モデル化された商品データやイベントデータが特定の金融商品区分や取引イベント区分に合致するかを自動判定（Boolean 判定）する関数群。
- **金融商品適格性判定** (`cdm.product.qualification`): **112 関数**（例: `Qualify_InterestRateSwap_FixedFloat`, `Qualify_CreditDefaultSwap`, `Qualify_Transaction_ZeroCoupon` 等）
- **取引イベント適格性判定** (`cdm.event.qualification`): **35 関数**（例: `Qualify_Execution`, `Qualify_Novation`, `Qualify_Allocation` 等）

### 4.3 取引ライフサイクル・イベント生成 (Lifecycle / Event Processing): 113 関数 (8.5%)
取引の発生（Execution）から配分（Allocation）、清算（Clearing）、更改（Novation）、解約（Termination）、リセット（Reset）等のライフサイクル状態遷移を実行する関数群。
- **取引イベント生成・状態更新** (`cdm.event.common`): **100 関数**（例: `Create_Execution`, `Create_Novation`, `Process_Allocation` 等）
- **命令合成・リセット処理** (`cdm.event.instructioncomposition*`): **13 関数**（`cdm.event.instructioncomposition.reset` 10関数: `Create_AdjustObservationDatesInstruction`, `Create_AdjustPeriodInstruction`, `UpdateResetCompositionState` 等、`cdm.event.instructioncomposition` 3関数）

### 4.4 商品・共通ユーティリティ (Product & Base Utility): 111 関数 (8.4%)
- **日時計算・営業日判定** (`cdm.base.datetime`): **37 関数**（営業日調整 `AdjustDateToFollowingBusinessDay`, `ShiftBusinessDays`、日付リスト生成 `ExpandMissingObservationDates`, `BuildRepeatedDateList` 等）
- **商品テンプレート・スケジュール・決済共通** (`cdm.product.*`): **33 関数**
- **その他基盤・ヘルパー関数**: **41 関数**

### 4.5 金利・日数・数値計算 (Calculation & Math): 80 関数 (6.0%)
